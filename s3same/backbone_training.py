...
class ResNet18Module(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = {'scheduler': OneCycleLR(optimizer, max_lr=self.learning_rate, 
                                             steps_per_epoch=int(len(train_dataloader)/self.trainer.accumulate_grad_batches), epochs=self.trainer.max_epochs),
                     'interval': 'step'}
...
if __name__ == "__main__":
    ...
    model = ResNet18Module(num_classes=num_classes, learning_rate=LR, weight_decay=WEIGHT_DECAY)
    
    print(f"Running on {device}")
    logger = TensorBoardLogger("tb_logs", name=f"{dataset}_r18_teacher_cs_v1")

    trainer = pl.Trainer(accelerator=device, devices=1, max_epochs=MAX_EPOCHS, 
                  callbacks=[ModelCheckpoint(monitor='val_loss'), LearningRateMonitor(logging_interval='step')], log_every_n_steps=5, logger=logger)
